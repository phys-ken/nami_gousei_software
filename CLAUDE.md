...
`TEST_CELL_SIZE.md` が現行のテスト指示書（cellSize 機能 + 既存退行確認の統合版）。
旧 Phase 1/1.1 用テスト書は `old_files/BROWSER_TEST_INSTRUCTIONS_PHASE1.md` に保存。

## ロジックのユニットテスト（Node.js）

```bash
node --test tests/wave.test.js tests/renderer.test.js
```

- `wave.js`: 全メソッド（setVertex / getY / getYAtTime / getSnapshot / clear / toJSON / fromJSON）を 30 ケースで検証
- `renderer.js`: 静的メソッド `computeCanvasSize` を 16 ケースで検証
- 合計 46 ケース。`problems.js`・`editor.js` はブラウザ Canvas API に依存するためブラウザでのみ動作確認可能
