...
AI エージェントから波の重ね合わせ問題（PNG画像群）を自動生成するための、ローカル専用 REST API。
...
`js/*.js` には一切手を入れていない。ブラウザでもサーバーでも同じ描画ロジックが動く。
...
# 2. 依存関係をインストール
#    Google Drive 同期下では node_modules がロックされて失敗するため、
#    ここでは Drive 外（C:\Users\croma\.node_caches\wave-problem-api\）に置く
...
```powershell
cd "g:\マイドライブ\#授業用\2026授業用フォルダ\010_G物理基礎\010_Ⅰ期\000_重ね合わせ設問作成ソフト"
node api_server.js
```
...
| `WAVE_API_NODE_MODULES` | `C:\Users\croma\.node_caches\wave-problem-api\node_modules` | 依存関係の置き場所 |
...
```json
{
  ...
  "answerValue": null,
  "files": {
    "question": [{"path": "...question_1.png"}, ...],
    "answer":   [{"path": "...answer_1.png"}, ...],
    "ref":      [{"path": "...ref_1.png"}, ...],
    "choices": [
      {"path": "...choice_1_correct.png", "isCorrect": true,  "label": "①", "originalIndex": 0},
      {"path": "...choice_2.png",         "isCorrect": false, "label": "②", "originalIndex": 2},
      ...
    ],
...
`inline: true` 時は `path` の代わりに `dataUrl` フィールドが入る。
...
API サーバー追加で `js/` ファイルは一切変更していない。次を確認:
1. `node --test tests/wave.test.js tests/renderer.test.js tests/random.test.js` が 64/64 通過するか
2. `python server.py` 単独で `http://localhost:8000` を開いて再現するか

---

## 既存資産の参照

- 計画書: `C:\Users\croma\.claude\plans\api-ai-ai-agent-api-api-zip-abstract-candle.md`
- バックエンド実装: `api/` 配下
